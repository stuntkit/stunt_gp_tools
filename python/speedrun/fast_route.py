...
parser.add_argument("filename", help="name of the save file", nargs='?', type=str, default='setup.bin')
parser.add_argument("-n", "--count", help="ncount of best tracks to diplay", nargs='?', type=int, default=3)
...
class ArcadePoint():
    def __init__(self):
        self.lap = PointTime(2*60*60, '...') # default time for "..." player, 2 mins
...
def main():
    ...
    if not os.path.exists(filename) or not os.path.isfile(filename):
        ...
        G = nx.DiGraph()
        G.add_nodes_from(["start", '01', '02', '04', '05', "06", "07", "08", "09", "10", "14", "15", "16", "17", "18", "19", "20", "21", "22", "end"])
        ...
        if len(shortests) < 0:
            ...
            if len(shortests) < args.count:
                print('found only {} shortest paths, but {} were requested'.format(len(shortests), args.count), file=sys.stderr)
            ...
            for i, path in enumerate(shortests, start = 1):
                ...
                for current_point in range(1, len(path) - 1):
                    time = G.get_edge_data(path[current_point], path[current_point+1])['weight']
                    total += time
            ...
            print('         20    06    02         ')
            print()
            print('See https://github.com/Halamix2/stunt_gp_formats/wiki/Tracks to see what track names corresponds to each ID')
